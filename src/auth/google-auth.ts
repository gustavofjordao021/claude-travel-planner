...
export async function handleAuthError(error: any): Promise<boolean> {
	if (
		...
		try {
			...
			if (credentials.refresh_token) {
				try {
					...
					if (newToken) {
						...
						logger.info(
							"Token refreshed successfully without requiring re-authentication"
						);
						return true;
					}
				} catch (refreshError) {
			...
			google.options({ auth });
			logger.info(
				"Authentication refreshed successfully via full re-auth flow"
			);
			return true;
		} catch (refreshError) {
			logger.error(
 ...
 */
export async function tryAuthWithAlternativePorts(): Promise<OAuth2Client> {
	...
	for (const port of alternativePorts) {
		...
		const tempKeysPath = path.join(
			__dirname,
			`../../.temp-gcp-oauth-${port}.json`
		);
		...
		try {
			...
			if (keyFile.installed && keyFile.installed.redirect_uris) {
				keyFile.installed.redirect_uris = keyFile.installed.redirect_uris.map(
					(uri: string) => uri.replace(/:[0-9]+\//, `:${port}/`)
				);
			} else if (keyFile.web && keyFile.web.redirect_uris) {
				keyFile.web.redirect_uris = keyFile.web.redirect_uris.map(
					(uri: string) => uri.replace(/:[0-9]+\//, `:${port}/`)
				);
			}
	...
	throw new Error(
		"Authentication failed with all alternative ports. Please update your OAuth credentials in Google Cloud Console."
	);
}
 ...
 */
export async function authenticateAndSaveCredentials(): Promise<OAuth2Client> {
	if (!fs.existsSync(gcpKeysPath)) {
		logger.error(
			"GCP keys not found. Please create your credentials in Google Cloud then copy `gcp-oauth.keys.json` into your project directory."
		);
		process.exit(1);
	}
	...
	try {
		...
		let authPromiseResolve: (value: OAuth2Client) => void;
		let authPromiseReject: (reason?: any) => void;
		const authPromise = new Promise<OAuth2Client>((resolve, reject) => {
			authPromiseResolve = resolve;
			authPromiseReject = reject;
		});

		const startAuthCallbackServer = (port: number) => {
		...
		if (isRegisteredPortAvailable) {
			...
			logger.info(
				`Using registered port ${registeredPort} for authentication...`
			);
			...
			for (const port of alternativePorts) {
				...
				try {
					...
					if (keyFile.installed && keyFile.installed.redirect_uris) {
						keyFile.installed.redirect_uris =
							keyFile.installed.redirect_uris.map((uri: string) =>
								uri.replace(/:[0-9]+\//, `:${port}/`)
							);
					} else if (keyFile.web && keyFile.web.redirect_uris) {
						keyFile.web.redirect_uris = keyFile.web.redirect_uris.map(
							(uri: string) => uri.replace(/:[0-9]+\//, `:${port}/`)
					...
					if (!portAvailable) {
						logger.info(
							`Alternative port ${port} is also in use, trying next...`
						);
						continue;
					}
					...
					logger.info(
						`Failed to authenticate with port ${port}: ${error.message}`
					);
...
export async function initializeAuth(): Promise<OAuth2Client> {
	try {
		...
		const isExpired =
			!expiryDate || new Date().getTime() > expiryDate - 5 * 60 * 1000;
		...
		if (isExpired && credentials.refresh_token) {
			...
			try {
				...
				logger.warn(
					"Token refresh failed, starting fresh authentication",
					error
				);
				return await authenticateAndSaveCredentials();
			}
