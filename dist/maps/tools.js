...
export async function geocode(args) {
    try {
        checkApiKey();
        const validArgs = validateArgs(args, ["address"]);
 ...
 */
export async function placesNearby(args) {
    try {
        checkApiKey();
 ...
 */
export async function distanceMatrix(args) {
    try {
        checkApiKey();
        const validArgs = validateArgs(args, [
            "origins",
...
export async function directions(args) {
    try {
        ...
        const { origin, destination, waypoints = [], mode = TravelMode.DRIVING, } = validArgs;
        const params = {
            origin,
            destination,
            mode,
 ...
 */
export async function timezone(args) {
    try {
        checkApiKey();
        const validArgs = validateArgs(args, ["location"]);
        ...
        const result = {
            ...
            local_time: new Date((timestamp + response.data.dstOffset + response.data.rawOffset) * 1000).toISOString(),
        };
        return {
            content: [
                {
